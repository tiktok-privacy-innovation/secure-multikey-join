...
template <typename Fn, typename... Args,
          typename R = std::invoke_result_t<
              Fn, const std::shared_ptr<yacl::link::Context>&, Args...>,
          std::enable_if_t<!std::is_same_v<R, void>, int> = 0>
std::vector<R> simulate_brpc(size_t npc, Fn&& fn, Args&&... args) {
  auto lctxs =
      yacl::link::test::SetupBrpcWorld(fmt::format("sim.{}", npc), npc);
...
template <typename Fn, typename... Args,
          typename R = std::invoke_result_t<
              Fn, const std::shared_ptr<yacl::link::Context>&, Args...>,
          std::enable_if_t<std::is_same_v<R, void>, int> = 0>
void simulate_brpc(size_t npc, Fn&& fn, Args&&... args) {
  auto lctxs =
      yacl::link::test::SetupBrpcWorld(fmt::format("sim.{}", npc), npc);
