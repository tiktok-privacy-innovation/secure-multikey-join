...
[[maybe_unused]] static void MultiplyFixedScalarInplace(
  ...
  const size_t num_coeff = ct.poly_modulus_degree();

  for (size_t k = 0; k < ct.size(); ++k) {
    using namespace seal::util;
    auto ptr = ct.data(k);
    for (size_t l = 0; l < modulus.size(); ++l) {
      auto fixed_mul = ntt_tables[l].inv_degree_modulo();

      multiply_poly_scalar_coeffmod(ptr, num_coeff, fixed_mul, modulus[l], ptr);
      ptr += num_coeff;
    }
  }
}
...
LWEPacker::LWEPacker(size_t nt) : num_th_(nt) {}

void LWEPacker::PackLWEs(absl::Span<const PhantomLWECt> lwes,
                         const GaloisKeys &galois,
                         const seal::SEALContext &context, RLWECt &out) {
  size_t num_lwes = lwes.size();
  size_t poly_degree{0};
  SPU_ENFORCE(IsValidLWEArray(lwes, &poly_degree),
  ...
  // Step 1: cast all LWEs to RLWEs
  std::vector<RLWECt> rlwes(num_lwes);
#pragma omp parallel for num_threads(num_th_)
  ...
  seal::Evaluator evaluator(context);
  size_t depth = 1;
  while (depth <= num_lwes) {
    using namespace seal::util;
    size_t n = num_lwes / depth;
    size_t h = n / 2;
    depth <<= 1;
...
#pragma omp parallel for num_threads(num_th_)
  ...
  // Step 2 to remove the extra factor (i.e., N/num_lwes) from Step 2.
  size_t log2N = absl::bit_width(poly_degree) - 1;
  size_t log2Nn = absl::bit_width(poly_degree / num_lwes) - 1;
...
void UnpackRLWE(const RLWECt &rlwe, const GaloisKeys &galois,
                const seal::SEALContext &context, absl::Span<RLWECt> out) {
  SPU_ENFORCE(context.parameters_set());
  SPU_ENFORCE(seal::is_metadata_valid_for(galois, context));
  ...
  // FFT-like method to merge RLWEs into one RLWE.
  seal::Evaluator evaluator(context);
  size_t depth = absl::bit_width(num_ct) - 1;

  out[0] = rlwe;
  MultiplyFixedScalarInplace(out[0], context);
...
#pragma omp parallel for
    for (size_t k = 0; k < h; ++k) {
      ...
      SPU_ENFORCE(c0.size() > 0);
      SPU_ENFORCE(c1.size() > 0);

      evaluator.apply_galois_inplace(c1, 1 + (poly_degree >> j), galois);

      evaluator.add(c0, c1, out[k]);
      evaluator.sub(c1, c0, out[k + h]);

      NegacyclicRightShiftInplace(out[k + h], poly_degree - (1UL << j),
                                  context);
    }
  }
}
