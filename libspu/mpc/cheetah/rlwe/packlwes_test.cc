...
TEST_P(PackLWEsTest, Unpack) {
  ...
  NdArrayRef array = ring_randbit(field, {poly_N});
  ...
  RLWECt rlwe;
  {
    ...
    CATCH_SEAL_ERROR(encryptor.encrypt_symmetric(pt, rlwe));
    InvNttInplace(rlwe, *N_context_);
  }

  std::vector<RLWECt> unpacked(poly_N);
  spu::mpc::cheetah::UnpackRLWE(rlwe, *galois_, *N_context_,
                                absl::MakeSpan(unpacked));
  for (size_t k = 0; k < poly_N; ++k) {
    NttInplace(unpacked[k], *N_context_);
    RLWEPt dec;
    decryptor.decrypt(unpacked[k], dec);
    ...
    ASSERT_EQ(array.data<scalar_t>()[k], coefficients[0]);

    for (size_t i = 1; i < poly_N; ++i) {
      ASSERT_EQ(0, coefficients[i]);
    }
  }
}
...
TEST_P(PackLWEsTest, Packer) {
  ...
  size_t num_rep = 8;
  std::vector<RLWECt> packed(num_rep);
  // LWEPacker packer(8);
  omp_set_nested(1);
...
#pragma omp parallel for num_threads(4)
  for (size_t i = 0; i < num_rep; ++i) {
    packer.PackLWEs(absl::MakeSpan(n_lwes), *galois_, *N_context_, packed[i]);
  }

  printf("pack %zd LWEs took %f s\n", num_lwes * num_rep,
         timer.CountMs() / 1e3);

  EXPECT_EQ(packed[0].size(), 2);
  EXPECT_FALSE(packed[0].is_ntt_form());
  EXPECT_EQ(packed[0].poly_modulus_degree(), poly_N);
  EXPECT_EQ(packed[0].coeff_modulus_size(), N_ms_helper_->coeff_modulus_size());

  N_evaluator.transform_to_ntt_inplace(packed[0]);
  RLWEPt dec;
  decryptor.decrypt(packed[0], dec);
  InvNttInplace(dec, *N_context_);
