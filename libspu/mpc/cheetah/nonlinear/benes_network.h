...
class BenesNetwork {
 public:
  ...
  template <typename T>
  void eval(absl::Span<T> src) const {
    SPU_ENFORCE_EQ((int64_t)src.size(), input_size_);
    eval<T>(log_aligned_size_, 0, 0, src);
  }
 ...
 private:
  ...
  template <typename T>
  void eval(int64_t n, int64_t lvl_p, int64_t perm_idx,
            absl::Span<T> src) const {
    int levels, i, j, x, s;
    std::vector<T> bottom1;
    std::vector<T> top1;
    int values = src.size();
    ...
    if (values == 2) {
      if (n == 1) {
        if (get_gate_type(lvl_p, perm_idx)) {
          temp = src[0];
          src[0] = src[1];
          src[1] = temp;
        }
      } else if (get_gate_type(lvl_p + 1, perm_idx)) {
        temp = src[0];
        src[0] = src[1];
        src[1] = temp;
      }
    ...
    if (values == 3) {
      ...
      if (get_gate_type(lvl_p + 1, perm_idx)) {
        temp = src[1];
        src[1] = src[2];
        src[2] = temp;
      }
      if (get_gate_type(lvl_p + 2, perm_idx)) {
        temp = src[0];
        src[0] = src[1];
        src[1] = temp;
      }
    ...
    for (i = 0; i < values - 1; i += 2) {
      int s = get_gate_type(lvl_p, perm_idx + i / 2);
      for (j = 0; j < 2; ++j) {
    ...
    eval(n - 1, lvl_p + 1, perm_idx, absl::MakeSpan(bottom1));
    eval(n - 1, lvl_p + 1, perm_idx + values / 4, absl::MakeSpan(top1));

    for (i = 0; i < values - 1; i += 2) {
      s = get_gate_type(lvl_p + levels - 1, perm_idx + i / 2);
      for (j = 0; j < 2; ++j) {
        x = benes_right_cycle_shift((i | j) ^ s, n);
        if (x < values / 2)
          src[i | j] = bottom1[x];
