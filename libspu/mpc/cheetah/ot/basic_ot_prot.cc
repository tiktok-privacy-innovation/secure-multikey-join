...
BasicOTProtocols::BasicOTProtocols(std::shared_ptr<yacl::link::Context> link,
                                   CheetahOtKind kind)
    : BasicOTProtocols(std::make_shared<Communicator>(link), kind) {}
...
std::array<NdArrayRef, 3> BasicOTProtocols::CorrelatedAndTriple(
    FieldType field, const Shape &shape, size_t k) {
  int64_t numel = shape.numel();
  SPU_ENFORCE(numel > 0);
  SPU_ENFORCE(k >= 1 and k <= SizeOf(field) * 8, "invalid k={}", k);

  const size_t ot_msg_width = k;
  ...
  DISPATCH_ALL_FIELDS(field, [&]() {
    ...
    std::vector<uint8_t> a(numel);
    std::vector<ring2k_t> v(numel);
    std::vector<ring2k_t> u(numel);
    // random choice a is 1-bit
    // random messages b, v and u are k-bit
    if (0 == Rank()) {
      ferret_receiver_->RecvRMRC(/*choice*/ absl::MakeSpan(a),
                                 absl::MakeSpan(u), ot_msg_width);
      ferret_sender_->SendRMRC(absl::MakeSpan(v),
                               absl::MakeSpan(&AND_xb[0], numel), ot_msg_width);
      ferret_sender_->Flush();
    } else {
      ferret_sender_->SendRMRC(absl::MakeSpan(v),
                               absl::MakeSpan(&AND_xb[0], numel), ot_msg_width);
    ...
    pforeach(0, numel, [&](int64_t i) {
      AND_xb[i] = AND_xb[i] ^ v[i];
      // broadcast to k-bit AND
      ring2k_t a_broadcast = 0;
      for (size_t j = 0; j < k; ++j) {
        a_broadcast = (a_broadcast << 1) | a[i];
      }

      AND_xa[i] = a[i] & 1;
      AND_xc[i] = (a_broadcast & AND_xb[i]) ^ u[i] ^ v[i];
    });
  });
  return {AND_a, AND_b, AND_c};
}
...
NdArrayRef BasicOTProtocols::Multiplexer(const NdArrayRef &msg,
  ...
  return DISPATCH_ALL_FIELDS(field, [&]() {
    ...
    pforeach(0, size, [&](int64_t i) {
      corr_data[i] = _msg[i] * (1 - 2 * (select[i] & 1));
    });

    if (Rank() == 0) {
      ferret_sender_->SendCAMCC(corr_data, sent);
      ferret_sender_->Flush();
      ferret_receiver_->RecvCAMCC(absl::MakeSpan(select), recv);
    } else {
      ferret_receiver_->RecvCAMCC(absl::MakeSpan(select), recv);
      ferret_sender_->SendCAMCC(corr_data, sent);
      ferret_sender_->Flush();
    ...
    pforeach(0, size, [&](int64_t i) {
      recv[i] =
          _msg[i] * static_cast<ring2k_t>(select[i] & 1) - sent[i] + recv[i];
    });
...
NdArrayRef BasicOTProtocols::Multiplexer(const NdArrayRef &msg,
  ...
  std::vector<uint8_t> sel(msg.numel());

  pforeach(0, msg.numel(), [&](int64_t i) {
    sel[i] = select.data<uint8_t>()[i * select.elsize()];
  });

  return Multiplexer(msg, absl::MakeSpan(sel));
}
