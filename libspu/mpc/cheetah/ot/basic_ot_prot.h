...
class BasicOTProtocols {
 public:
  ...
  explicit BasicOTProtocols(std::shared_ptr<yacl::link::Context> link,
                            CheetahOtKind kind);
  ...
  NdArrayRef Multiplexer(const NdArrayRef &msg,
  ...
  std::array<NdArrayRef, 3> AndTriple(FieldType field, const Shape &shape,
                                      size_t k);
  // 1 <= k <= field size
  // [a, B, C] where a is 1bit and B, C are k-bits
  // such that C = a ^ B
  std::array<NdArrayRef, 3> CorrelatedAndTriple(FieldType field,
  ...
  std::shared_ptr<Communicator> GetCommunicator() { return conn_; }
