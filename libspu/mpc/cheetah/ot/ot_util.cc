...
std::uint8_t Pack8Bits(absl::Span<const std::uint8_t> b8) {
  SPU_ENFORCE_EQ(b8.size(), 8UL);

  std::uint64_t mask = 0x0101010101010101ULL;
  std::uint64_t unpack;
  // 8 x 8b = 64b
  std::memcpy(&unpack, b8.data(), sizeof(std::uint64_t));
  ...
  return static_cast<std::uint8_t>(pack);
}
...
void Unpack8Bits(uint8_t packed, absl::Span<std::uint8_t> b8) {
  ...
  std::memcpy(b8.data(), &unpack, 8);
}

std::uint16_t Pack16Bits(absl::Span<const std::uint8_t> b16) {
  SPU_ENFORCE_EQ(b16.size(), 16UL);

  uint128_t mask = (static_cast<uint128_t>(0x0101010101010101ULL) << 64);
  mask |= 0x0101010101010101ULL;

  uint128_t unpack;
  // 16 x 8b = 128b
  std::memcpy(&unpack, b16.data(), sizeof(uint128_t));
  ...
  return static_cast<std::uint16_t>(pack);
}

uint128_t PackAtMost128Bits(absl::Span<const std::uint8_t> bits) {
  SPU_ENFORCE_LE(bits.size(), 128UL);

  uint128_t ret = 0;

  std::size_t idx = bits.size();
  for (; idx >= 16; idx -= 16) {
    ret = (ret << 16) | Pack16Bits(bits.subspan(idx - 16, 16));
  }

  for (; idx >= 8; idx -= 8) {
    ret = (ret << 8) | Pack8Bits(bits.subspan(idx - 8, 8));
  }

  for (; idx >= 1; idx -= 1) {
    ret = (ret << 1) | bits[idx - 1];
  }
