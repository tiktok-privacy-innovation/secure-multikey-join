...
TEST_P(BasicOTProtTest, SingleB2A) {
  ...
  utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
    ...
    if (ctx->Rank() == 0) {
      ashr0 = ot_prot.B2ASingleBitWithSize(bshr0, 2);
    } else {
      ashr1 = ot_prot.B2ASingleBitWithSize(bshr1, 2);
    }
...
TEST_P(BasicOTProtTest, CorrelatedBitwiseAnd) {
  size_t kWorldSize = 2;
  Shape shape = {1L << 22};
  FieldType field = std::get<0>(GetParam());
  auto ot_type = std::get<1>(GetParam());
  ...
  for (int i : {0, 1}) {
    ...
    rhs0[i] = ring_rand(field, shape).as(boolean_t);
    rhs1[i] = ring_rand(field, shape).as(boolean_t);

    DISPATCH_ALL_FIELDS(field, [&]() {
      ring2k_t mask = makeBitsMask<ring2k_t>(bw);
      NdArrayView<ring2k_t> L(lhs[i]);
      NdArrayView<ring2k_t> R0(rhs0[i]);
      NdArrayView<ring2k_t> R1(rhs1[i]);

      pforeach(0, shape.numel(), [&](int64_t j) { L[j] &= mask; });
      pforeach(0, shape.numel(), [&](int64_t j) { R0[j] &= mask; });
      pforeach(0, shape.numel(), [&](int64_t j) { R1[j] &= mask; });
    });
  }
  ...
  utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
    ...
    auto o = ot_prot.CorrelatedBitwiseAnd(lhs[r].clone(), rhs0[r].clone(),
                                          rhs1[r].clone());
    out0[r] = o[0];
    out1[r] = o[1];
    if (r == 0) {
      ...
      printf("CBwAnd sent %zd %zd B\n", sent, recv);
      printf("CBwAnd sent %f b per \n", (sent + recv) * 8. / shape.numel());
    }
  });

  auto expected0 =
      ring_and(ring_xor(lhs[0], lhs[1]), ring_xor(rhs0[0], rhs0[1]));
  auto expected1 =
      ring_and(ring_xor(lhs[0], lhs[1]), ring_xor(rhs1[0], rhs1[1]));
  auto got0 = ring_xor(out0[0], out0[1]);
  auto got1 = ring_xor(out1[0], out1[1]);

  DISPATCH_ALL_FIELDS(field, [&]() {
    NdArrayView<ring2k_t> e0(expected0);
    NdArrayView<ring2k_t> e1(expected1);
    NdArrayView<ring2k_t> g0(got0);
    NdArrayView<ring2k_t> g1(got1);
...
TEST_P(BasicOTProtTest, BitwiseAnd) {
  ...
  utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
    ...
    if (r == 0) {
      sent = ctx->GetStats()->sent_bytes - sent;
      recv = ctx->GetStats()->recv_bytes - recv;
      printf("BwAnd sent %zd %zd B\n", sent, recv);
      printf("BwAnd sent %f b per \n", (sent + recv) * 8. / shape.numel());
    }
  });
...
TEST_P(BasicOTProtTest, CorrelatedAndTriple_Specific_Width) {
  size_t kWorldSize = 2;
  Shape shape = {1 << 22};
  FieldType field = std::get<0>(GetParam());
  auto ot_type = std::get<1>(GetParam());
  std::array<NdArrayRef, 3> corr_triple[2];

  size_t bw = SizeOf(field) * 8;
  for (size_t k : {2UL, 3UL, bw}) {
    utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
      auto conn = std::make_shared<Communicator>(ctx);
      BasicOTProtocols ot_prot(conn, ot_type);
      ...
      size_t sent = ctx->GetStats()->sent_bytes;
      size_t recv = ctx->GetStats()->recv_bytes;
      corr_triple[ctx->Rank()] = ot_prot.CorrelatedAndTriple(field, shape, k);
      sent = ctx->GetStats()->sent_bytes - sent;
      recv = ctx->GetStats()->recv_bytes - recv;
      if (ctx->Rank()) {
        printf("Batched CorrelatedAndTriple k = %zd, %fbits per\n", k,
               (sent + recv) * 8. / shape.numel());
      }
    });
    ...
    DISPATCH_ALL_FIELDS(field, [&]() {
      ring2k_t max = static_cast<ring2k_t>(1) << k;
      if (k == bw) {
        max = -1;
      }
      ...
      for (int64_t i = 0; i < shape.numel(); ++i) {
        ...
        ring2k_t a_bit = (a0[i] ^ a1[i]);
        ring2k_t a_broadcast = 0;
        for (size_t j = 0; j < k; ++j) {
          a_broadcast = (a_broadcast << 1) | a_bit;
        }
        ring2k_t e = a_broadcast & (b0[i] ^ b1[i]);
        ring2k_t c = (c0[i] ^ c1[i]);
        ASSERT_EQ(e, c);
      }
    });
...
TEST_P(BasicOTProtTest, PrivateMulx) {
  ...
  utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
    ...
    sent = ctx->GetStats()->sent_bytes - sent;
    recv = ctx->GetStats()->recv_bytes - recv;
    printf("sent %f b recv %f b\n", (sent * 8.) / shape.numel(),
           (recv * 8.0) / shape.numel());
  });
