...
TEST_P(FerretCOTTest, OSN) {
  ...
  size_t num_gates = 2;
  std::vector<uint64_t> input(2);
  input[0] = 10;
  input[1] = 20;

  std::vector<uint128_t> wires0(num_gates + 1);
  std::vector<uint128_t> wires1(num_gates + 1);

  std::vector<uint8_t> controls(num_gates);
  std::vector<uint128_t> recv_masks(num_gates + 1);
  ...
  DISPATCH_ALL_FIELDS(field, [&]() {
    utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
      ...
      size_t sent = ctx->GetStats()->sent_bytes;
      if (rank == 0) {
        ferret.SendRMCC(absl::MakeSpan(wires0), absl::MakeSpan(wires1), bw);
        ferret.Flush();
      } else {
        ferret.RecvRMCC(absl::MakeSpan(controls), absl::MakeSpan(recv_masks),
                        bw);
      }
      sent = ctx->GetStats()->sent_bytes - sent;
      printf("RMCC %fb per\n", sent * 8. / wires0.size());
...
TEST_P(FerretCOTTest, ChosenMsgChosenChoice_N2) {
  size_t kWorldSize = 2;
  int64_t N = 2;
  int64_t n = 22 * (1L << 22);
  auto field = std::get<0>(GetParam());
  ...
  DISPATCH_ALL_FIELDS(field, [&]() {
    ...
    utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
      ...
      if (rank == 0) {
        ...
        ferret.RecvCMCC(absl::MakeSpan(choices), N, absl::MakeSpan(selected),
                        bw);
      }
      sent = ctx->GetStats()->sent_bytes - sent;
      printf("CMCC %fb per\n", sent * 8. / n);
