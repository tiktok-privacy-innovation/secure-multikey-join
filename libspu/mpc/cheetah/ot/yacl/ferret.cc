...
struct YaclFerretOt::Impl {
 private:
  ...
  _yacl::MITCCRHExp<8> mitccrh_exp_{};
 ...
 public:
  ...
  void SendCorrelatedMsgChosenChoice(absl::Span<const T> corr,
    ...
    SendRandCorrelatedMsgChosenChoice(rcm_output.data(), n);
    const size_t _kOTBatchSize = lcm(8 * sizeof(T), bit_width);

    std::vector<uint128_t> pad(2 * _kOTBatchSize);
    // std::array<uint128_t, 2 * _kOTBatchSize> pad;
    std::vector<T> corr_output(_kOTBatchSize);

    size_t eltsize = 8 * sizeof(T);
    bool packable = eltsize > (size_t)bit_width;

    size_t packed_size = CeilDiv(_kOTBatchSize * bit_width, eltsize);
    ...
    for (size_t i = 0; i < n; i += _kOTBatchSize) {
      size_t this_batch = std::min(_kOTBatchSize, n - i);
      for (size_t j = 0; j < this_batch; ++j) {
        pad[2 * j] = rcm_output[i + j];
        pad[2 * j + 1] = rcm_output[i + j] ^ ferret_->GetDelta();
      }
  ...
  void RecvCorrelatedMsgChosenChoice(absl::Span<const uint8_t> choices,
    ...
    const size_t _kOTBatchSize = lcm(8 * sizeof(T), bit_width);
    std::vector<uint128_t> pad(_kOTBatchSize);
    // std::array<uint128_t, _kOTBatchSize> pad;
    std::vector<T> corr_output(_kOTBatchSize);
