...
CuckooIndexHelper::CuckooIndexHelper(const CuckooIndex& cch, int32_t numel)
    : num_bin_(cch.bins().size()), num_sample_(numel) {
  YACL_ENFORCE(num_sample_ > 0 && num_sample_ <= num_bin_,
               "num_sample={}, num_bin={}", num_sample_, num_bin_);

  sample_to_bin_map_.resize(num_sample_);
  bin_to_sample_perm_.resize(num_bin_);

  std::fill_n(bin_to_sample_perm_.data(), num_bin_, -1);
  for (int32_t b = 0; b < num_bin_; ++b) {
    const auto& bin = cch.bins()[b];
    if (!bin.IsEmpty()) {
      sample_to_bin_map_[bin.InputIdx()] = b;
      bin_to_sample_perm_[b] = bin.InputIdx();
    }
  }

  std::vector<int32_t> empty_slots(num_bin_ - num_sample_);
  std::iota(empty_slots.begin(), empty_slots.end(), num_sample_);
  std::shuffle(empty_slots.begin(), empty_slots.end(),
               std::default_random_engine());

  auto next_pos = empty_slots.cbegin();
  for (int32_t b = 0; b < num_bin_; ++b) {
    if (bin_to_sample_perm_[b] == -1) {
      bin_to_sample_perm_[b] = *next_pos++;
    }
  }

  std::vector<int32_t> inv(num_bin_);
  for (int32_t b = 0; b < num_bin_; ++b) {
    inv[bin_to_sample_perm_[b]] = b;
  }
  bin_to_sample_perm_ = inv;

  // SanityCheck
  for (int32_t i = 0; i < num_sample_; ++i) {
    YACL_ENFORCE_EQ(bin_to_sample_perm_[i], sample_to_bin_map_[i]);
  }
}
