...
void Rr22OprfSender::Init(const std::shared_ptr<yacl::link::Context>& lctx,
  ...
  if (mode_ == Rr22PsiMode::FastMode) {
    ...
    SPDLOG_DEBUG("recv baxos seed finished");
    ...
    SPDLOG_DEBUG("paxos_size:{}", paxos_size_);
    // vole send function
...
std::vector<uint128_t> Rr22OprfSender::SendFast(
  ...
  uint128_t ws = 0;
  if (malicious_) {
    SPDLOG_DEBUG("malicious version");
    // generate ws
  ...
  SPDLOG_DEBUG("recv paxos solve ...");
  auto paxos_solve_v = RecvChunked<uint128_t>(lctx, paxos_size_);
  SPDLOG_DEBUG("recv paxos solve finished. bytes:{}",
  ...
  SPDLOG_DEBUG("begin b xor delta a");
  ...
  SPDLOG_DEBUG("end b xor delta a");
  return outputs;
}
...
std::vector<uint128_t> Rr22OprfSender::SendLowComm(
  ...
  SPDLOG_DEBUG("paxos_solve_u64 size:{}", paxos_solve_u64.size());
...
std::vector<uint128_t> Rr22OprfSender::Eval(
  ...
  SPDLOG_DEBUG("paxos decode finished");
  b_.clear();
...
std::vector<uint128_t> Rr22OprfSender::Eval(
  ...
  SPDLOG_DEBUG("paxos decode (mode:{}) ...",
...
void Rr22OprfReceiver::Init(const std::shared_ptr<yacl::link::Context>& lctx,
  ...
  if (mode_ == Rr22PsiMode::FastMode) {
    ...
    baxos_.Init(init_size, bin_size_, kPaxosWeight, ssp_,
                okvs::PaxosParam::DenseType::GF128, baxos_seed);
    paxos_size_ = baxos_.size();
    SPDLOG_DEBUG("baxos_size:{}", paxos_size_);
    // c + b = a * delta
    // vole recv
    ...
    // vole recv function
    SPDLOG_DEBUG("use SilentVoleReceiver");
...
std::vector<uint128_t> Rr22OprfReceiver::RecvFast(
  ...
  auto oprf_eval_proc = std::async([&] {
    SPDLOG_DEBUG("begin compute self oprf");
    ...
    if (malicious_) {
      SPDLOG_DEBUG("call Davis-Meyer hash");
      DavisMeyerHash(outputs_span, inputs, outputs_span);
    } else {
      SPDLOG_DEBUG("call aes crhash");
      aes_crhash.Hash(outputs_span, outputs_span);
    }
    SPDLOG_DEBUG("end compute self oprf");
  });

  SPDLOG_DEBUG("begin p xor a");
