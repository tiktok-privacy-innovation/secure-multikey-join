...
class CuckooIndexHelper {
 public:
  CuckooIndexHelper(const CuckooIndex& cch, int32_t numel);

  const std::vector<int32_t>& sample_to_bin_map() const {
    return sample_to_bin_map_;
  }

  // Permute the Cuckoo Hash Table and place the non-empty slots to the front.
  // y[i] <= CuckooIndexTable[perm[i]] for i = 0, 1, .., num_sample_ - 1
  const std::vector<int32_t>& bin_to_sample_perm() const {
    return bin_to_sample_perm_;
  }

  int32_t num_bin() const { return num_bin_; }

  int32_t num_sample() const { return num_sample_; }

 private:
  int32_t num_bin_;
  int32_t num_sample_;

  std::vector<int32_t> sample_to_bin_map_;

  std::vector<int32_t> bin_to_sample_perm_;
};
